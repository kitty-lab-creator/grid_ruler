  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="w-7 h-7 rounded-full bg-slate-100 hover:bg-slate-200 active:bg-slate-300 flex items-center justify-center border border-slate-200 text-black transition-colors cursor-pointer ml-0.5"
        title={`重設為建議值 (${defaultPpi.toFixed(1)})`}
        aria-label="重設建議比例"
      >
        <RotateCcw className="w-3.5 h-3.5 stroke-[2] text-black" />
      </button>
    </div>
  );
};
